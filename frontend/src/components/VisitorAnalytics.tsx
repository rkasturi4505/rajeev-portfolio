  ...
  const loadAnalytics = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/visitor-sessions/analytics`,
      );

      setAnalytics(response.data);
    } catch (error) {
      console.error("Visitor analytics loading failed:", error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      <div className="visitor-analytics">
        ...
        <div className="analytics-card">
          ...
          <h1 className="text-value">{analytics.topBrowser}</h1>
        </div>

        <div className="analytics-card">
          <h3>Top Country</h3>

          <h1 className="text-value">{analytics.topCountry}</h1>
        </div>
      </div>
